...
@Component({
  ...
  template: `
      <tui-island [hoverable]="true">
          <ng-container *ngIf="detail; else empty">
              ...
              />
              <p class="tui-island__category tui-space_top-7 tui-space_bottom-0">TJM</p>
              <p class="tui-space_top-0">
              <span class="tui-island__title tui-space_top-0 daily-rate">
                  {{ detail.lastInvoice.dailyRate | currency:'EUR' }}
              </span>
                  <span class="tui-island__paragraph">
                      ({{ detail.lastInvoice.month | date:'MMMM y' | titlecase }})
                  </span>
              </p>
...
export class MissionDetailComponent {
  @Input({required: true}) detail: MissionDetail | null;
}
